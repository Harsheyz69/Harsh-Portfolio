...
const ProjectCard = ({ title, tech, description, index }) => (
    <motion.div
        ...
        className="bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-lg border border-gray-100 dark:border-gray-700 hover:shadow-2xl transition-all duration-300 group flex flex-col h-full"
    >
        <div className="p-8 flex flex-col h-full">
            <div className="flex justify-between items-start mb-6">
                <h3 className="text-2xl font-bold text-dark dark:text-light group-hover:text-primary transition-colors">
                    {title}
                </h3>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
                {tech.map((t, i) => (
                    <span
                        key={i}
                        className="px-4 py-1.5 text-sm font-medium rounded-full bg-primary/10 text-dark dark:text-primary"
                    >
                        {t}
                    </span>
                ))}
